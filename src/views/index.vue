...
function reSend(index) {
  question.value = list.value[index-1].text;
  send();
}
...
<template>
  <div class="page">
    ...
    <div class="begintitle">
      <h1 v-show="!list.length" @click="send">Kathe的ChatGPT</h1>
      <div v-show="list.length" style="padding: 12px;font-size: 18px;">new chat</div>
      <img v-if="false" src="../../public/wx.png" alt="" />
    </div>

    <div id="myList">
      <div :class="item.type === 0 ? 'problemList' : 'answerList'" v-for="(item, index) in list">
        <img class="listImg" :src="item.avatar" alt="" />
        <div v-if="false" v-highlight v-html="item.text" class="listText"></div>
        <div style="max-width: calc(100% - 45px);">
          <v-md-preview :text="item.text" />
        </div>
        <div style="display: block;width: 100%;text-align: left;padding: 0 0 0 54px;">
          <el-button type="text"  @click="copyToClipboard(item.text)">复制</el-button>
          <el-button v-if="item.text === '出错了，请重试！'" type="text" @click="reSend(index)">重试</el-button>
        </div>
      </div>

      <div v-show="loading" class="answerList">
        <img class="listImg" src="/log.png" alt="" />
        <img v-if="false" class="addin" src="/loading.gif" alt="" />
    ...
    <div class="inputbox">
      ...
      <el-input
          ...
          placeholder="输入你的指令"
      ><template #append>
        <el-button type="text" icon="Position" @click="send"></el-button>
      </template></el-input>
